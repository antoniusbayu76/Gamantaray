...
from utils.general import (check_img_size, non_max_suppression, scale_coords,
                           xyxy2xywh, increment_path, set_logging, time_synchronized)
from utils.plots import plot_one_box
from utils.torch_utils import select_device, TracedModel, load_classifier, apply_classifier

def detect(save_img=False):
    sources = opt.source if isinstance(opt.source, list) else [opt.source]
    ...
    # Initialize capture objects for multiple sources
    captures = [cv2.VideoCapture(source) for source in sources]
    ...
    while True:
        for idx, cap in enumerate(captures):
            ret, frame = cap.read()
            if not ret:
                print(f"Failed to grab frame from source {idx}")
                continue
            ...
            for i, det in enumerate(pred):  # detections per image
                im0 = frame.copy()
                p, s, frame = f'Camera {idx + 1}', '', frame
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', nargs='+', type=str, default=['0', '1'], help='source')  # multiple sources
    ...
    parser.add_argument('--nosave', action='store_true', help='Do not save images or labels')
    parser.add_argument('--project', default='runs/detect', help='Save results to project/dir')
    parser.add_argument('--name', default='exp', help='Save results to project/dir/name')
