...
lowerRed2 = np.array([163,43,142])  #[0, 124, 53]     #[163,43,142]
upperRed2= np.array([180,255,255]) #[74, 255, 255]   #[180,255,255]
lowerRed = np.array([0, 78, 67])  
upperRed = np.array([6, 255, 177])
...
lowerGreen2 = np.array([69,92,0])  
upperGreen2 = np.array([179, 255, 255])
...
while True:
    ...
    ##Area of interest
    cv2.rectangle(img, (0,0), (img.shape[1],blockAtas), (0,255,255), thickness=-1)  #y=90  #y=110   ###
    cv2.rectangle(img, ((img.shape[1]//2)-blockTengah,0), ((img.shape[1]//2)+blockTengah,img.shape[0]), (0,255,255), thickness=-1) ###
    cv2.line(img, (img.shape[1]//5,blockAtas-50), (-5,blockAtas+10), (0,255,255), thickness=blockKiri) ###
    cv2.line(img, (img.shape[1]-img.shape[1]//5,blockAtas-50), (img.shape[1]+5,blockAtas+10), (0,255,255), thickness=blockKanan) ###
   #------------------------------------------------------------------------------------------------------------ 
   #roi tambahan
    cv2.rectangle(img, (0,(img.shape[0]//2)+blockBawah), (img.shape[1],img.shape[0]), (0,255,255), thickness=-1)
    ...
    cv2.fillPoly(img, pts=[points], color=(0, 255, 255))#merah
    ...
    cv2.fillPoly(img, pts=[points2], color=(0, 255, 255))#hijau
    ...
    cxDOT = (cxH + cxM) // 2
    cyDOT = (cyH + cyM) // 2
    cv2.circle(img, (cxDOT,cyDOT), 10, (0,0,255), thickness=2)
