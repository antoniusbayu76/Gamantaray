...
#List masking merah
listLowRed = [np.array([163,43,142]),np.array([0,81,127])] # [np.array([0, 111, 109])]
listUpRed = [np.array([180,255,255]),np.array([4,244,249])] #[np.array([3, 190, 178])] 
...
def detect_red(image, lower_red, upper_red):
    ...
    cv2.rectangle(mask_red, (image.shape[1]//2,0), (image.shape[1],image.shape[0]), (0,0,0), thickness=-1)
...
def detect_green(image, lower_green, upper_green):
    ...
    cv2.rectangle(mask_green, (0,0), (image.shape[1]//2,image.shape[0]), (0,0,0), thickness=-1) 
...
def main():
    ...
    while True:
        ...
        red_detected = detect_red(frame, listLowRed[nRed], listUpRed[nRed])
        green_detected = detect_green(frame, listLowGreen[nGreen], listUpGreen[nGreen])
        ...
        if not red_detected and not green_detected:
            ...
            if len(contours3) > 0:
                ...
                if M['m00']!=0:
                    cxK = int(M['m10']/M['m00'])
                    cyK = int(M['m01']/M['m00'])
                    cv2.rectangle(frame, (cxK-50,cyH-50), (cxK+50,cyK+50), (255,255,255), 1)
            else :
                cxK = frame.shape[1]//2
                cyK = frame.shape[0]//2
